...
def draw_cube(position, size, color):
    ...
    pyglet.graphics.draw(24, gl.GL_QUADS, ('v3f', vertices), ('c3f', colors))

class Framebuffer:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.texture = pyglet.image.Texture.create(width, height, internalformat=gl.GL_RGBA)
        fbo_id = gl.GLuint()
        gl.glGenFramebuffers(1, byref(fbo_id))
        self.id = fbo_id.value
        gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, self.id)
        gl.glFramebufferTexture2D(gl.GL_FRAMEBUFFER, gl.GL_COLOR_ATTACHMENT0, self.texture.target, self.texture.id, 0)

        depth_buffer = gl.GLuint()
        gl.glGenRenderbuffers(1, byref(depth_buffer))
...
class CarWindow(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        self.fbo.bind()
        gl.glViewport(0, 0, self.mirror_width, self.mirror_height)
        self.set_3d_projection(self.mirror_width, self.mirror_height)
        gl.glLoadIdentity()
        gl.glRotatef(self.player.rotation[0], 1, 0, 0)
        gl.glRotatef(self.player.rotation[1] + 180, 0, 1, 0)
        gl.glTranslatef(-self.player.position[0], -self.player.position[1], -self.player.position[2])
        ...
        gl.glLoadIdentity()
        gl.glRotatef(self.player.rotation[0], 1, 0, 0)
        gl.glRotatef(self.player.rotation[1], 0, 1, 0)
        gl.glTranslatef(-self.player.position[0], -self.player.position[1], -self.player.position[2])
        gl.glClear(gl.GL_COLOR_BUFFER_BIT | gl.GL_DEPTH_BUFFER_BIT)
        self.draw_scene()
    ...
    def draw_rear_view_hud(self):
        ...
        gl.glColor3f(1, 1, 1)
        gl.glEnable(self.fbo.texture.target)
        gl.glBindTexture(self.fbo.texture.target, self.fbo.texture.id)
        ...
        gl.glDisable(self.fbo.texture.target)
